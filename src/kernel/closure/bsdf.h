...
ccl_device int bsdf_sample(KernelGlobals *kg, const ShaderData *sd, const ShaderClosure *sc, float randu, float randv, float3 *eval, float3 *omega_in, differential3 *domega_in, float *pdf)
{
	...
	switch(sc->type) {
		...
		case CLOSURE_BSDF_PHONG_ID:
			label = bsdf_phong_sample(sc, sd->Ng, sd->I, sd->dI.dx, sd->dI.dy, randu, randv,
				eval, omega_in, &domega_in->dx, &domega_in->dy, pdf);
			break;
	...
	if(dot(sd->Ng, omega_in) >= 0.0f) {
		switch(sc->type) {
			...
			case CLOSURE_BSDF_ASHIKHMIN_SHIRLEY_ANISO_ID:
				...
				break;
			case CLOSURE_BSDF_PHONG_ID:
				eval = bsdf_phong_eval_reflect(sc, sd->I, omega_in, pdf);
				break;
			case CLOSURE_BSDF_ASHIKHMIN_VELVET_ID:
		...
		switch(sc->type) {
			...
			case CLOSURE_BSDF_ASHIKHMIN_SHIRLEY_ANISO_ID:
				...
				break;
			case CLOSURE_BSDF_PHONG_ID:
				eval = bsdf_phong_eval_transmit(sc, sd->I, omega_in, pdf);
				break;
			case CLOSURE_BSDF_ASHIKHMIN_VELVET_ID:
